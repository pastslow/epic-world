...
export class EnemyAttackService {
   ...
   public animAttack(monster: DynamicBody, castedAbility): void {
      ...
      if (monster.anims.currentAnim.key === monsterTargetOrigin.name + '_' + 'attack' && monster.anims.currentFrame.isLast) {
         ...
         debugger;
         monsterTargetOrigin.combatAttributes.pauseStartTime = GameScene.time.now;
   ...
   public handleTargetOverlap(arrow: DynamicBody, overlappedEntity: DynamicBody): void {
   ...
   public handleAmmoGroundHit(arrow: DynamicBody): void {
      arrow.destroy();
   }
